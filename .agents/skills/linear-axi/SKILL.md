...
8. Handle remote-browser loopback callbacks.
   Completion criterion: if the user approves Linear OAuth and the live browser lands on `http://127.0.0.1:14582/oauth/callback?...` with `This site can't be reached`, keep the still-running `auth login` process alive and replay that full callback URL from the repo machine with `curl`. Then read the waiting CLI output and verify `.env` was written. Do not paste the callback code or resulting tokens in the final answer.
